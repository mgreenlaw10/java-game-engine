A 2D game engine written from scratch in Java. There is a game player and an editor, and maps can be constructed and loaded to the game from the editor without touching source code. The project is compiled, so you can put 'java -cp bin src.Main' in terminal in the project directory to run. Additionally, there is a Makefile so you make also choose to run 'make' with no target to compile and run. If all else fails, you can directly type the compile and run scripts as folows:
